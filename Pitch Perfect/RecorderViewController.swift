...
class RecorderViewController: UIViewController, AVAudioRecorderDelegate{
    ...
    @IBAction func pauseBtn(sender: UIButton) {
        if audioRecorder.recording == true {
            audioRecorder.pause()
            recordButtonOutlet.setImage(UIImage(named: "pause"), forState: .Normal)
        } else {
            audioRecorder.record()
            recordButtonOutlet.setImage(UIImage(named: "microphone-iphone"), forState: .Normal)
        }
    }
    ...
    func audioRecorderDidFinishRecording(recorder: AVAudioRecorder, successfully flag: Bool) {
        if flag {
            if let title = recorder.url.lastPathComponent {
                ...
                performSegueWithIdentifier("secondVC", sender: audioData)
            }
        }
    }
